...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const loadedChats = loadChatMessages()
              ...
              {selectedStory ? (
                <div className="bg-white rounded-2xl shadow-lg p-8">
                  <button
                    ...
                    className="mb-4 px-4 py-2 bg-slate-500 hover:bg-slate-600 text-white rounded-lg font-semibold"
                  >
                    ← Kembali
                  </button>
                  <div className="border-l-4 border-purple-500 pl-6">
                    <h2 className="text-3xl font-bold text-slate-800 mb-2">{selectedStory.title}</h2>
                    <p className="text-slate-600 mb-4">
                      Oleh <strong>{selectedStory.username}</strong> • {new Date(selectedStory.timestamp).toLocaleDateString("id-ID", { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </p>
                    <div className="bg-slate-50 p-6 rounded-xl">
                      <p className="text-slate-700 whitespace-pre-wrap leading-relaxed">{selectedStory.content}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="bg-white rounded-2xl shadow-lg p-8">
                  {stories.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-slate-600">Belum ada cerita yang diunggah</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {stories.map(story => (
                        <div key={story.id} className="p-4 border-2 border-slate-200 rounded-xl hover:shadow-lg transition cursor-pointer">
                          <div className="flex justify-between items-start mb-2">
                            <div className="flex-1">
                              <p className="font-semibold text-slate-800">{story.title}</p>
                              <p className="text-sm text-slate-500">{story.username} • {new Date(story.timestamp).toLocaleDateString("id-ID")}</p>
                            </div>
                            <button 
                          ...
                          <p className="text-slate-600 line-clamp-2">{story.content}</p>
                        </div>
                      ))}
                    </div>
                  )}
